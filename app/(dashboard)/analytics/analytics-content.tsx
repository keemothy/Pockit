...
function EmptyState() {
  return (
    ...
    >
      ...
      <div>
        <h2 className="text-[17px] font-bold text-slate-900">No spending data yet</h2>
        <p className="mt-1 max-w-sm text-sm text-slate-400">
          Connect a bank account and your real spending breakdown will appear here automatically.
        </p>
      </div>
      <Link
        href="/wallet"
  ...
  return (
    <div className="flex items-end justify-between gap-2 pt-2">
      {categories.slice(0, 7).map((cat, i) => (
        <div key={cat.label} className="group flex flex-1 flex-col items-center gap-2">
          <div className="relative w-full" style={{ height: 160 }}>
            <motion.div
              className="absolute bottom-0 left-0 right-0 rounded-t-lg"
              style={{ background: `linear-gradient(180deg, ${cat.color}, ${cat.color}88)` }}
              initial={{ height: 0 }}
              animate={{ height: `${Math.max((cat.amount / maxAmount) * 160, 4)}px` }}
              transition={{ delay: 0.1 + i * 0.055, duration: 0.52, ease: [0.22, 1, 0.36, 1] }}
            />
          ...
          <span className="text-[10px] text-slate-400">{cat.label.split(' ')[0]}</span>
        </div>
      ))}
    </div>
  );
          ...
          >
            <div className="flex items-baseline justify-between">
              <div>
                ...
                <p className="mt-1 font-mono text-3xl font-black text-slate-900" style={{ fontVariantNumeric: "tabular-nums" }}>
              ...
              {categories.map((cat, i) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-slate-400">{cat.percent}%</span>
                  </div>
                  <p className="mt-2 font-mono text-xl font-black text-slate-900" style={{ fontVariantNumeric: "tabular-nums" }}>
                    ${cat.amount.toFixed(0)}
                  </p>
                  <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
                    <motion.div
                      ...
                      transition={{ delay: 0.38 + i * 0.055, duration: 0.52, ease: [0.22, 1, 0.36, 1] }}
                    />
                  </div>
                </motion.div>
              ))}
