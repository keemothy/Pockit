...
function Donut({ card }: { card: WalletCard }) {
  const stops = card.categories.reduce(
    (accumulator, category) => {
      ...
      const segment = (category.amount / card.currentBalance) * 100;
...
export default function WalletPage() {
  ...
  function focusCard(cardId: number) {
  ...
  function addCard(event: FormEvent<HTMLFormElement>) {
    ...
    const card: WalletCard = {
      id: Date.now(), name: name.trim(), issuer: 'POCKIT CARD', lastFour: normalizedLastFour,
    ...
    <div className="w-full min-w-0 bg-white text-[#121926]">
      <main className="w-full min-w-0">
          ...
          <div className="mt-5 flex flex-wrap justify-end gap-2">
            ...
            <button type="button" onClick={() => setIsModalOpen(true)} className="rounded-xl bg-[#2865e9] px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700">＋ &nbsp; Add credit cards</button>
          </div>
